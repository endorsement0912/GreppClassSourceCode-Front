...
public class LeaveFormValidator implements Validator<LeaveForm> {
  ...
  public void validate(LeaveForm form) {
    ...
    Optional<MemberDto> memeber = memberDao.selectById(conn, form.getUserId());

    if(memeber.isEmpty()) throw new ValidException("존재하지 않는 사용자 입니다.");
    if(memeber.get().getLeave()) throw new ValidException("이미 탈퇴한 회원입니다.");
